#Ce fichier contient l'interface graphique du jeu
...
#definition de chaque couleur en fonction du numero de la pièce
def Couleur(i):
    switcher = {
        0: "white",
        1: "#FF52EA",       #pink
...
def InterfaceGraphique(grid):
    ...
    EndPositionXPremierRectangle = StartPositionXPremierRectangle + NombreDeCerclesX *(DiamètreCercle+EspaceEntreCercle) - EspaceEntreCercle + 20
    EndPositionYPremierRectangle = StartPositionYPremierRectangle + NombreDeCerclesY *(DiamètreCercle+EspaceEntreCercle) - EspaceEntreCercle + 20
    ...
    Label_Possibilitee = canvas.create_text(StartPositionXPremierRectangle+3, 50, font=("Courrier", 10), text="Nombre de possibilités :", anchor = 'w')
    Label_PieceDispo = canvas.create_text(StartPositionXDeuxièmeRectangle, 50, font=("Courrier", 10), text="Pièces disponibles", anchor = 'w')
    Label_Difficulte = canvas.create_text(StartPositionXPremierRectangle, EndPositionYPremierRectangle + 25, font=("Courrier", 10), text="Choisir un niveau de difficulté :", anchor = 'w')



    def Actualisation(grid, pieces):                #fonction qui permet de faire l'actualisation de la grille lors du changement du niveau
        ...
        for Z in range(len(pieces)):  # pour chaque pièces restante
            try:
                ...
                pieceActu = pieces[Z]                   #dans le cas où on récupère déjà la configuration correcte de la pièce à afficher
                TailleYPiece = pieceActu.shape[0]
                TailleXPiece = pieceActu.shape[1]
            ...
            for Y in range(TailleYPiece):  # pour Y dans les coordonnées en Y de la matrice de la piece
                for X in range(TailleXPiece):  # pour X dans les coordonnées en X de la matrice de la piece
                    if (pieceActu[Y][X] != 0):          #On affiche seulement la pièce et non pas sa matrice entière
                        CouleurPiece = Couleur(pieceActu[Y][X])                  #la grid ne peut pas être égal à 0, les autres chiffres correspondent à la piece elle-meme,
                        ...
                        if(Z <= 3):
                            coordonneeDebutX = 700+Z*EspaceXEntrePiece + X * (DiamètreCercle/ 3 + EspaceEntreCercle/ 3)
                            coordonneeDebutY = PositionYPremierCercle + Y * (DiamètreCercle / 3 + EspaceEntreCercle / 3)
                            coordonneeFinX = 700+Z*EspaceXEntrePiece + DiamètreCercle / 3 + X * (DiamètreCercle / 3 + EspaceEntreCercle / 3)
                            coordonneeFinY = PositionYPremierCercle + DiamètreCercle / 3 + Y * (DiamètreCercle / 3 + EspaceEntreCercle / 3)
                        elif (Z <= 7):
                            coordonneeDebutX = 700+(Z-4)*EspaceXEntrePiece + X * (DiamètreCercle / 3 + EspaceEntreCercle / 3)
                            coordonneeDebutY = PositionYPremierCercle+EspaceYEntrePiece + Y * (DiamètreCercle / 3 + EspaceEntreCercle / 3)
                            coordonneeFinX = 700+(Z-4)*EspaceXEntrePiece + DiamètreCercle / 3 + X * (DiamètreCercle / 3 + EspaceEntreCercle / 3)
                            coordonneeFinY = PositionYPremierCercle + EspaceYEntrePiece + DiamètreCercle / 3 + Y * (
                            DiamètreCercle / 3 + EspaceEntreCercle / 3)
                        elif (Z <= 11):
                            coordonneeDebutX = 700+(Z-8)*EspaceXEntrePiece + X * (DiamètreCercle / 3 + EspaceEntreCercle / 3)
                            coordonneeDebutY = PositionYPremierCercle + EspaceYEntrePiece*2 + Y * (DiamètreCercle / 3 + EspaceEntreCercle / 3)
                            coordonneeFinX = 700+(Z-8)*EspaceXEntrePiece + DiamètreCercle / 3 + X * (DiamètreCercle / 3 + EspaceEntreCercle / 3)
                        ...
                        OvalePieceRestante.append(
                            canvas.create_oval(coordonneeDebutX, coordonneeDebutY, coordonneeFinX, coordonneeFinY,
                                               outline=CouleurPiece, fill = CouleurPiece))  # créer des cercles et les sauvegarde dans une liste
        ...
        for Y in range(5):
            for X in range(11):
                ...
                if (grid[Y, X] == 0):  # pour remplir avec des cases vides c'est à dire des ronds avec coutour blanc remplis avec du gris
                    ...
                    CouleurPiece = Couleur(grid[Y, X])  # grid va renvoyer le nombre correspond au coordonnées de la case dans la matrice
                    Ovale.append(canvas.create_oval(PositionXPremierCercle + X * (DiamètreCercle + EspaceEntreCercle),
                                       PositionYPremierCercle + Y * (DiamètreCercle + EspaceEntreCercle),
                                       PositionXPremierCercle + DiamètreCercle + X * (
                                                   DiamètreCercle + EspaceEntreCercle),
    ...
    #éxécute cette fonction lorsque la difficulté a changé, celle-ci a été modifiée par le joueur
    def difficulty_changed(event):
        ...
        def Level_changed(event):
            ...
            NiveauSelectionne = comboBoxNiveau.get()[7:]        #on prend les dernière lettres du string. "Niveau 4" comporte 8 lettres, on va supprimé les 7 première pour avoir seulement le numéro du niveau
            NiveauEntier = int(NiveauSelectionne)               #convertie le niveau en entier afin de pouvoir l'afficher dans la grille
            ...
            def LancementIA():
                if(NiveauEntier != 0):                          #si un niveau est selectionné
                    ...
                    Actualisation(grid, ListePiecesMise)            #affiche la configuration des pièces posées et leurs ordre de placement



            # bouton
            bouton = Button(window, text="Lancer l'IA", command=LancementIA)
            bouton.place(x=580, y=EndPositionYPremierRectangle + 13)  # permet de placer le bouton
        ...
        if (comboBoxDifficulte.get() == "STARTER"):
            for n in range(1, 12 + 1):              # le "+1" est ajouté car la fonction range(x, y) va de x à y non inclue [x, y[
                Niveau.append("Niveau " + str(n))   # ajoute le texte niveau dans une liste, la fonction str permet de convertir en string
        ...
        if (Niveau):  # si la liste n'est pas vide
            ...
            comboBoxNiveau.bind('<<ComboboxSelected>>', Level_changed)          #exécute du code situé dans l'event "Level_changed" lorsque la difficultée est changée
    #combobox
    #creation d'un groupe de plusieurs difficultées
    Difficulte = ('STARTER', 'JUNIOR', 'EXPERT', 'MASTER', 'WIZARD')
    comboBoxDifficulte = ttk.Combobox(window,values=Difficulte, width=20)         #créer un combobox et y ajoute les données
    ...
    comboBoxDifficulte.bind('<<ComboboxSelected>>', difficulty_changed) #exécute du code situé dans l'event "difficulty_changed" lorsque la difficultée est changée
    ...
    canvas.create_rectangle(StartPositionXPremierRectangle, StartPositionYPremierRectangle, EndPositionXPremierRectangle, EndPositionYPremierRectangle, fill =CouleurPremierRectange, outline ="#3a5a92", width = 2)
    canvas.create_rectangle(StartPositionXDeuxièmeRectangle, StartPositionYDeuxièmeRectangle, EndPositionXDeuxièmeRectangle, EndPositionYDeuxièmeRectangle, fill =CouleurPremierRectange, outline ="#3a5a92", width = 2)
